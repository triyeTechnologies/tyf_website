...
const STYLES = `
  ...
  td a.mail{border-bottom:1px solid rgba(129,64,220,.42)}
  ...
  .chip.contacted{background:rgba(129,64,220,.16);border-color:rgba(129,64,220,.5);color:var(--neon-deep)}
  .chip.qualified{background:transparent;border-color:rgba(129,64,220,.5);color:var(--neon-deep)}
...
function sparkline(daily) {
  ...
  return html`<svg viewBox="0 0 100 30" preserveAspectRatio="none" aria-hidden="true">
    <polyline points="${raw(points.join(' '))}" fill="none" stroke="#9A66E3" stroke-width="1.6"
      stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>
    <polyline points="${raw(`0,30 ${points.join(' ')} 100,30`)}" fill="rgba(129,64,220,.14)" stroke="none"/>
  </svg>`;
}
